...
export function agentJwtSecretCheck(configPath?: string): CheckResult {
  if (readAgentJwtSecretFromEnv(configPath)) {
    return {
      ...
      status: "pass",
      message: "ZEPHYR_AGENT_JWT_SECRET is set in environment",
    };
  }
  ...
  if (fileSecret) {
    return {
      name: "Agent JWT secret",
      status: "warn",
      message: `ZEPHYR_AGENT_JWT_SECRET is present in ${envPath} but not loaded into environment`,
  ...
  return {
    name: "Agent JWT secret",
    status: "fail",
    message: `ZEPHYR_AGENT_JWT_SECRET missing from environment and ${envPath}`,
    canRepair: true,
    repair: () => {
      ensureAgentJwtSecret(configPath);
    },
    repairHint: `Run with --repair to create ${envPath} containing ZEPHYR_AGENT_JWT_SECRET`,
  };
}
