...
const DATA_DIR_OPTION_HELP =
  "Zephyr Nexus data directory root (isolates state from ~/.zephyr-nexus)";

program
  .name("zephyr")
  .description("Zephyr Nexus CLI — setup, diagnose, and configure your instance")
  .version("0.2.7");
...
program
  ...
  .option("--run", "Start Zephyr Nexus immediately after saving config", false)
  .action(onboard);

program
  .command("doctor")
  .description("Run diagnostic checks on your Zephyr Nexus setup")
  .option("-c, --config <path>", "Path to config file")
  .option("-d, --data-dir <path>", DATA_DIR_OPTION_HELP)
...
program
  ...
  .option("--filename-prefix <prefix>", "Backup filename prefix", "zephyr-nexus")
...
program
  .command("run")
  .description("Bootstrap local setup (onboard + doctor) and run Zephyr Nexus")
  .option("-c, --config <path>", "Path to config file")
  .option("-d, --data-dir <path>", DATA_DIR_OPTION_HELP)
...
heartbeat
  ...
  .option("--api-base <url>", "Base URL for the Zephyr Nexus server API")
