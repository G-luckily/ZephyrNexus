/**
 * esbuild configuration for building the @zephyr-nexus/cli for npm.
 *
 * Bundles all workspace packages (@zephyr-nexus/*) into a single file.
...
// to npm and resolved at runtime (e.g. @zephyr-nexus/server uses dynamic import).
...
for (const p of workspacePaths) {
  ...
  for (const name of Object.keys(pkg.dependencies || {})) {
    if (externalWorkspacePackages.has(name)) {
      externals.add(name);
    } else if (!name.startsWith("@zephyr-nexus/")) {
      externals.add(name);
    }
  }
