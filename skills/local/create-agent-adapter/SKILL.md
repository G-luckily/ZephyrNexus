...
## 2. Shared Types (`@zephyr-nexus/adapter-utils`)

All adapter interfaces live in `packages/adapter-utils/src/types.ts`. Import from `@zephyr-nexus/adapter-utils` (types) or `@zephyr-nexus/adapter-utils/server-utils` (runtime helpers).
...
```json
{
  "name": "@zephyr-nexus/adapter-<name>",
  "version": "0.0.1",
  "private": true,
  "type": "module",
  "exports": {
  ...
  "dependencies": {
    "@zephyr-nexus/adapter-utils": "workspace:*",
    "picocolors": "^1.1.1"
  },
  "devDependencies": {
    "typescript": "^5.7.3"
...
1. **Read config** — extract typed values from `ctx.config` using helpers (`asString`, `asNumber`, `asBoolean`, `asStringArray`, `parseObject` from `@zephyr-nexus/adapter-utils/server-utils`)
2. **Build environment** — call `buildPaperclipEnv(agent)` then layer in `ZEPHYR_RUN_ID`, context vars (`ZEPHYR_TASK_ID`, `ZEPHYR_WAKE_REASON`, `ZEPHYR_WAKE_COMMENT_ID`, `ZEPHYR_APPROVAL_ID`, `ZEPHYR_APPROVAL_STATUS`, `ZEPHYR_LINKED_ISSUE_IDS`), user env overrides, and auth token
...
| Variable | Source |
|----------|--------|
| `ZEPHYR_AGENT_ID` | `agent.id` |
| `ZEPHYR_COMPANY_ID` | `agent.companyId` |
| `ZEPHYR_API_URL` | Server's own URL |
| `ZEPHYR_RUN_ID` | Current run id |
| `ZEPHYR_TASK_ID` | `context.taskId` or `context.issueId` |
| `ZEPHYR_WAKE_REASON` | `context.wakeReason` |
| `ZEPHYR_WAKE_COMMENT_ID` | `context.wakeCommentId` or `context.commentId` |
| `ZEPHYR_APPROVAL_ID` | `context.approvalId` |
| `ZEPHYR_APPROVAL_STATUS` | `context.approvalStatus` |
| `ZEPHYR_LINKED_ISSUE_IDS` | `context.issueIds` (comma-separated) |
| `ZEPHYR_API_KEY` | `authToken` (if no explicit key in config) |
...
Pretty-prints stdout lines for `zephyr run --watch`. Use `picocolors` for coloring.
...
```ts
import { execute as myExecute, sessionCodec as mySessionCodec } from "@zephyr-nexus/adapter-my-agent/server";
import { agentConfigurationDoc as myDoc, models as myModels } from "@zephyr-nexus/adapter-my-agent";
...
import { parseMyAgentStdoutLine } from "@zephyr-nexus/adapter-my-agent/ui";
import { MyAgentConfigFields } from "./config-fields";
import { buildMyAgentConfig } from "@zephyr-nexus/adapter-my-agent/ui";
...
```ts
import { printMyAgentStreamEvent } from "@zephyr-nexus/adapter-my-agent/cli";
...
Import from `@zephyr-nexus/adapter-utils/server-utils`:
...
| `buildPaperclipEnv(agent)` | Standard `ZEPHYR_*` env vars |
...
- Package name: `@zephyr-nexus/adapter-<kebab-name>`
...
async function buildSkillsDir(): Promise<string> {
  ...
  const entries = await fs.readdir(ZEPHYR_SKILLS_DIR, { withFileTypes: true });
  for (const entry of entries) {
    if (entry.isDirectory()) {
      await fs.symlink(
        path.join(ZEPHYR_SKILLS_DIR, entry.name),
...
- `ZEPHYR_API_KEY` is injected by the server into the process environment, not the prompt
