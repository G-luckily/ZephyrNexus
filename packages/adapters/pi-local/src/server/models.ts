...
import type { AdapterModel } from "@zephyr-nexus/adapter-utils";
import { asString, runChildProcess } from "@zephyr-nexus/adapter-utils/server-utils";
...
function resolvePiCommand(input: unknown): string {
  const envOverride =
    typeof process.env.ZEPHYR_PI_COMMAND === "string" &&
    process.env.ZEPHYR_PI_COMMAND.trim().length > 0
      ? process.env.ZEPHYR_PI_COMMAND.trim()
      : "pi";
  return asString(input, envOverride);
}

const discoveryCache = new Map<string, { expiresAt: number; models: AdapterModel[] }>();
const VOLATILE_ENV_KEY_PREFIXES = ["ZEPHYR_", "npm_", "NPM_"] as const;
