...
// ---------------------------------------------------------------------------
// Logical execution-engine Port abstraction
// ---------------------------------------------------------------------------

/**
 * High-level request shape for a logical execution engine.
 * Today this is a thin wrapper around AdapterExecutionContext, but gives
 * the server a semantic place to hang additional cross-cutting concerns
 * (retry policy, routing decisions, tracing metadata, etc.).
 */
...
/**
 * Logical execution result; currently identical to AdapterExecutionResult
 * so existing adapter behavior remains the single source of truth.
 */
export type ExecutionEngineResult = AdapterExecutionResult;

/**
 * A logical execution engine port. In simple cases this is a direct
 * façade over a single ServerAdapterModule; more advanced engines may
 * fan out to multiple adapters or apply routing before invoking them.
 */
export interface ExecutionEnginePort {
  /** Logical engine type, e.g. "openclaw", "process", "http", "firecrawl". */
  type: string;
