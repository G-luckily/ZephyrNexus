...
server/heartbeat ──┘
  - executeRun 开始前：buildContext(scope) → 注入 context.zephyrContextBlock
...
- **Diff snapshot**：由 adapter 通过 `resultJson.zephyrDiffSnapshot` 返回（或 heartbeat 构造最小 snapshot），由 context-manager 写入 store。
...
5. 提供 **CLI 入口**，在 repo 根目录执行，默认写入 `./.zephyr-nexus/context.db`（可配置）。
...
1. **Build context 插入点**：`executeRun` 内，在 `context.zephyrWorkspace = ...` 之后、`adapter.execute()` 之前。
   - 使用 `executionWorkspace.cwd` 与已有 `context.issueId` / `context.taskKey` 构造 scope。
   - 调用 `context-manager` 的 `buildContext(store, scope)`；若 store 不可用或出错，则 `context.zephyrContextBlock = null` 或省略，不阻塞执行。
   - 将结果写入 `context.zephyrContextBlock`（字符串或结构化对象，由 adapter 消费）。
2. **Snapshot 写回插入点**：`adapter.execute()` 返回后，在 `setRunStatus` 之前或之后、`releaseIssueExecutionAndPromote` 之前。
   - 从 `adapterResult.resultJson?.zephyrDiffSnapshot` 取 snapshot；若无，则构造最小 snapshot（taskId, runId, workerId, touchedFiles: [] 等）。
   - 调用 `context-manager` 的 `writeDiffSnapshot(store, snapshot)`；store 路径由 `executionWorkspace.cwd` 推导（与 extractor 一致）。
3. **Store 实例**：heartbeat 内按 run 的 `executionWorkspace.cwd` 创建或复用 SqliteContextStore（路径 `cwd + '/.zephyr-nexus/context.db'`）；无 DB 时 builder 返回空 block，writeSnapshot 可 no-op 或写内存/跳过。
...
| 文件 | 操作 | 说明 |
|------|------|------|
| `package.json` | 新增 | name: @zephyr-nexus/context-manager，依赖 better-sqlite3（或 sql.js），无 db/server |
...
| 文件 | 操作 | 说明 |
|------|------|------|
| `package.json` | 新增 | name: @zephyr-nexus/context-extractor，依赖 @zephyr-nexus/context-manager, ts-morph，CLI 入口 bin |
...
| 文件 | 操作 | 说明 |
|------|------|------|
| `package.json` | 修改 | 增加依赖 "@zephyr-nexus/context-manager": "workspace:*" |
| `src/services/heartbeat.ts` | 修改 | 在 executionWorkspace 就绪后、adapter.execute 前：创建 store(cwd)，buildContext(store, scope)，注入 context.zephyrContextBlock；在 adapter.execute 返回后：从 adapterResult.resultJson?.zephyrDiffSnapshot 或构造最小 snapshot，writeDiffSnapshot(store, snapshot) |
...
2. **Phase 1.2**：在任意 TypeScript 仓库根执行 `pnpm exec context-extractor`（或类似），生成 `./.zephyr-nexus/context.db`，且 context-manager 能读出。
3. **Phase 1.3**：跑一次 heartbeat 执行 run，检查 context 中带 zephyrContextBlock，run 结束后 DB 中有对应 diff_snapshot 记录（可为最小 snapshot）。
