...
const env = {
  ...process.env,
  ZEPHYR_UI_DEV_MIDDLEWARE: "true",
};

if (tailscaleAuth) {
  env.ZEPHYR_DEPLOYMENT_MODE = "authenticated";
  env.ZEPHYR_DEPLOYMENT_EXPOSURE = "private";
  env.ZEPHYR_AUTH_BASE_URL_MODE = "auto";
  env.HOST = "0.0.0.0";
  console.log("[zephyr-nexus] dev mode: authenticated/private (tailscale-friendly) on 0.0.0.0");
} else {
...
async function maybePreflightMigrations() {
  ...
  if (process.env.ZEPHYR_MIGRATION_PROMPT === "never") return;

  const status = await runPnpm(
    ["--filter", "@zephyr-nexus/db", "exec", "tsx", "src/migration-status.ts", "--json"],
    { env },
  );
  if (status.code !== 0) {
  ...
  const autoApply = process.env.ZEPHYR_MIGRATION_AUTO_APPLY === "true";
...
if (mode === "watch") {
  env.ZEPHYR_MIGRATION_PROMPT = "never";
}
