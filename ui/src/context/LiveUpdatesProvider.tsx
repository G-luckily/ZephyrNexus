...
function describeIssueUpdate(
  ...
  if (typeof details.status === "string")
    changes.push(`状态变更：${tStatus(details.status)}`);
  ...
  if (details.reopened === true) {
    ...
    changes.push(from ? `从 ${tStatus(from)} 重开` : "已重开");
  }
  if (typeof details.title === "string") changes.push("标题已修改");
  if (typeof details.description === "string") changes.push("描述已修改");
...
function buildActivityToast(
  ...
  if (action === "issue.created") {
    return {
      title: `任务已创建 ${issue.ref}`,
      body: issue.title ? truncate(issue.title, 96) : undefined,
      tone: "success",
      action: { label: "查看任务", href: issue.href },
...
function buildJoinRequestToast(
  ...
  const label = requestType === "agent" ? "智能体" : "成员";

  return {
    title: `${label} 请求加入`,
    body: "有新的加入请求等待审批。",
    tone: "info",
    action: { label: "查看消息", href: "/inbox/new" },
...
function buildAgentStatusToast(
  ...
  const name = nameOf(agentId) ?? `智能体 ${shortId(agentId)}`;
  const title = status === "running" ? `${name} 已启动` : `${name} 异常`;
  ...
  return {
    ...
    body,
    tone,
    action: { label: "查看智能体", href: `/agents/${agentId}` },
...
function buildRunStatusToast(
  ...
  const agentId = readString(payload.agentId);
  const status = readString(payload.status);
  if (!runId || !agentId || !status || !TERMINAL_RUN_STATUSES.has(status))
    return null;
  ...
  const statusLabel =
    status === "succeeded"
      ...
      ? "超时"
      : "已取消";
  const title = `${name} 执行${statusLabel}`;

  let body: string | undefined;
  if (error) {
    body = truncate(error, 100);
  } else if (triggerDetail) {
    body = `触发来源：${triggerDetail}`;
  }
  ...
  return {
    ...
    action: { label: "查看执行", href: `/agents/${agentId}/runs/${runId}` },
...
export function LiveUpdatesProvider({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    ...
    const connect = () => {
      ...
      }/api/companies/${encodeURIComponent(selectedCompanyId)}/events/ws`;
