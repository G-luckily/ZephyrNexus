...
export interface OrgNode {
  ...
  status: string;
  reports: OrgNode[];
  /** P1-2: Adapter type (e.g. openclaw_gateway) for cost-aware filtering. */
  adapterType?: string;
  /** P1-2: Derived cost tier: "high" | "normal" for high-cost adapters. */
  costTier?: "high" | "normal";
}
...
export const agentsApi = {
  ...
  updatePermissions: (
    id: string,
    data: { canCreateAgents: boolean },
    companyId?: string
  ) => api.patch<Agent>(agentPath(id, companyId, "/permissions"), data),
  pause: (id: string, companyId?: string) =>
    api.post<Agent>(agentPath(id, companyId, "/pause"), {}),
  resume: (id: string, companyId?: string) =>
    api.post<Agent>(agentPath(id, companyId, "/resume"), {}),
  terminate: (id: string, companyId?: string) =>
    api.post<Agent>(agentPath(id, companyId, "/terminate"), {}),
  remove: (id: string, companyId?: string) =>
    api.delete<{ ok: true }>(agentPath(id, companyId)),
  listKeys: (id: string, companyId?: string) =>
    api.get<AgentKey[]>(agentPath(id, companyId, "/keys")),
  ...
  budgetSummary: (companyId: string, id: string) =>
    api.get<AgentBudgetSummary>(`/companies/${companyId}/agents/${id}/budget-summary`),
};
