import type { HeartbeatRun } from "@zephyr-nexus/shared";
...
export function RunActivityChart({ runs }: { runs: HeartbeatRun[] }) {
  ...
  const grouped = new Map<
    string,
    { succeeded: number; failed: number; other: number }
  >();
  for (const day of days)
    grouped.set(day, { succeeded: 0, failed: 0, other: 0 });
  ...
  const maxValue = Math.max(
    ...Array.from(grouped.values()).map(
      (v) => v.succeeded + v.failed + v.other
    ),
    1
  );
  const hasData = Array.from(grouped.values()).some(
    (v) => v.succeeded + v.failed + v.other > 0
  );
                ...
                >
                  {entry.succeeded > 0 && (
                    <div
                      ...
                      style={{ flex: entry.succeeded }}
                    />
                  )}
                  {entry.failed > 0 && (
                    <div
                      className="bg-red-500"
                      style={{ flex: entry.failed }}
                    />
                  )}
                  {entry.other > 0 && (
                    <div
                      className="bg-neutral-500"
                      style={{ flex: entry.other }}
                    />
                  )}
                </div>
              ) : (
...
export function PriorityChart({
  ...
  const maxValue = Math.max(
    ...Array.from(grouped.values()).map((v) =>
      Object.values(v).reduce((a, b) => a + b, 0)
    ),
    1
  );
  const hasData = Array.from(grouped.values()).some(
    (v) => Object.values(v).reduce((a, b) => a + b, 0) > 0
  );

  if (!hasData)
    return <p className="text-xs text-muted-foreground">No issues</p>;
        ...
        {days.map((day) => {
          ...
          return (
            <div
              ...
              className="flex-1 h-full flex flex-col justify-end"
              title={`${day}: ${
                entry.total > 0 ? Math.round(rate * 100) : 0
