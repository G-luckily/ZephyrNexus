...
import type { LiveEvent } from "@zephyr-nexus/shared";
...
function createFeedItem(
  ...
  const maxLength = options?.streamingKind
    ? MAX_STREAMING_TEXT_LENGTH
    : MAX_FEED_TEXT_LENGTH;
...
export function LiveRunWidget({ issueId, companyId }: LiveRunWidgetProps) {
  ...
  const runMetaByIdRef = useRef(
    new Map<string, { agentId: string; agentName: string }>()
  );
  ...
  useEffect(() => {
    for (const run of runs) {
      runMetaByIdRef.current.set(run.id, {
        agentId: run.agentId,
        agentName: run.agentName,
      });
    }
  ...
  return (
    <div className="rounded-lg border border-cyan-500/30 bg-background/80 overflow-hidden shadow-[0_0_12px_rgba(6,182,212,0.08)]">
      ...
      <div
        ref={bodyRef}
        className="max-h-[220px] overflow-y-auto p-2 font-mono text-[11px] space-y-1"
      >
        {recent.length === 0 && (
