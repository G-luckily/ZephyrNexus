...
const TASK_ROLE_OPTIONS: { value: TaskRole; label: string }[] = [
  { value: "", label: "No role" },
...
export function NewIssueDialog() {
  ...
  const [taskRole, setTaskRole] = useState<TaskRole>("");
  const [assigneeThinkingEffort, setAssigneeThinkingEffort] = useState("");
  const [assigneeChrome, setAssigneeChrome] = useState(false);
  const [useIsolatedExecutionWorkspace, setUseIsolatedExecutionWorkspace] =
    useState(false);
  ...
  useEffect(() => {
    ...
    if (newIssueDefaults.title) {
      ...
      setTaskRole((draft.taskRole ?? "") as TaskRole);
      setAssigneeThinkingEffort(draft.assigneeThinkingEffort ?? "");
      setAssigneeChrome(draft.assigneeChrome ?? false);
      setUseIsolatedExecutionWorkspace(
        draft.useIsolatedExecutionWorkspace ?? false
      );
    } else {
      setStatus(newIssueDefaults.status ?? "todo");
      setPriority(newIssueDefaults.priority ?? "");
  ...
  const handleProjectChange = useCallback(
    (nextProjectId: string) => {
      ...
      const policy = SHOW_EXPERIMENTAL_ISSUE_WORKTREE_UI
        ? nextProject?.executionWorkspacePolicy
        : null;
      executionWorkspaceDefaultProjectId.current = nextProjectId || null;
      setUseIsolatedExecutionWorkspace(
        Boolean(policy?.enabled && policy.defaultMode === "isolated")
      );
    },
    [orderedProjects]
  );
        ...
        <div className="flex items-center gap-1.5 px-4 py-2 border-t border-border flex-wrap shrink-0">
              ...
              <button className="inline-flex items-center gap-1.5 rounded-md border border-border px-2 py-1 text-xs hover:bg-accent/50 transition-colors">
                {currentPriority ? (
                  ...
                  <>
                    ...
                    <span className="text-foreground/80">优先级</span>
                  </>
                )}
              </button>
            </PopoverTrigger>
          ...
          <Popover>
            ...
            <PopoverContent className="w-44 p-1" align="start">
              {TASK_ROLE_OPTIONS.map((roleOption) => (
                <button
                  key={roleOption.value || "none"}
                  className={cn(
                    "flex items-center gap-2 w-full px-2 py-1.5 text-xs rounded hover:bg-accent/50",
                    roleOption.value === taskRole && "bg-accent"
                  )}
                  onClick={() => setTaskRole(roleOption.value)}
                >
                  {roleOption.label}
                </button>
              ))}
