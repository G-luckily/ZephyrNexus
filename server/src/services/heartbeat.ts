...
function wakeupRequestIssueId(request: typeof agentWakeupRequests.$inferSelect) {
  const payload = parseObject(request.payload);
  return (
    readNonEmptyString(payload.issueId) ??
    readNonEmptyString(parseObject(payload[DEFERRED_WAKE_CONTEXT_KEY]).issueId) ??
    null
  );
}
...
export function heartbeatService(db: Db) {
  ...
  const HIGH_COST_ADAPTER_TYPES = new Set(["openclaw_gateway"]);

  function isHighCostAdapter(adapterType: string): boolean {
    return HIGH_COST_ADAPTER_TYPES.has(adapterType);
  }
  ...
  function parseHeartbeatPolicy(agent: typeof agents.$inferSelect) {
    ...
    return {
      ...
      allowTimerForHighCostAdapter: asBoolean(heartbeat.allowTimerForHighCostAdapter, false),
    };
  }
  ...
  async function updateRuntimeState(
    ...
    if (additionalCostCents > 0 || hasTokenUsage) {
      ...
      const projectId = readNonEmptyString(ctx.projectId) ?? undefined;
  ...
  async function executeRun(runId: string) {
    ...
    if (Number.isFinite(budgetMonthlyCents) && budgetMonthlyCents > 0 && spentMonthlyCents >= budgetMonthlyCents) {
      ...
      if (failedRun) {
        await appendRunEvent(failedRun, 1, {
          ...
          stream: "system",
          level: "warn",
          message: "run blocked by budget guard",
          payload: {
            budgetMonthlyCents,
            spentMonthlyCents,
            agentId: agent.id,
    ...
    const issueBudgetCents =
      ...
      rawSettings !== null &&
      typeof (rawSettings as Record<string, unknown>).budgetCents === "number"
      ...
      if (issueSpentCents >= issueBudgetCents) {
        ...
        if (failedRun) {
          await appendRunEvent(failedRun, 1, {
            ...
            stream: "system",
            level: "warn",
            message: "run blocked by issue budget guard",
            payload: { budgetCents: issueBudgetCents, spent: issueSpentCents, issueId },
          });
          await releaseIssueExecutionAndPromote(failedRun);
        }
        await finalizeAgentStatus(agent.id, "failed");
    ...
    if (executionWorkspace.cwd) {
      try {
        ...
        const scope = {
          projectRoot: executionWorkspace.cwd,
          issueId: readNonEmptyString(context.issueId) ?? undefined,
          taskKey: readNonEmptyString(context.taskKey) ?? undefined,
        };
        const maxContextChars = getMaxContextBlockChars();
        const maxSnapshots = getMaxContextPriorSnapshots();
        const contextBlock = await buildContext(contextStore, scope, {
  ...
  async function enqueueWakeup(agentId: string, opts: WakeupOptions = {}) {
    ...
    // P0-1: High-cost adapters (e.g. openclaw_gateway) block timer/scheduler by default to avoid
    // background burn. Manual wakeup and comment-triggered wakeup are allowed. Opt-in via
    // runtimeConfig.heartbeat.allowTimerForHighCostAdapter = true to re-enable timer for this agent.
    if (
      source === "timer" &&
      isHighCostAdapter(agent.adapterType) &&
      !policy.allowTimerForHighCostAdapter
    ) {
      await writeSkippedRequest("high_cost_adapter.timer_blocked_by_default");
      return null;
    }
    ...
    if (issueId && !bypassIssueExecutionLock) {
      ...
      const outcome = await db.transaction(async (tx) => {
        ...
        if (!activeExecutionRun) {
          const sameAgentIssueRun = await tx
            .select()
            .from(heartbeatRuns)
            .where(
              and(
            ...
            .then((rows) => rows.find((row) => runIssueId(row) === issue.id) ?? null);

          if (sameAgentIssueRun) {
            activeExecutionRun = sameAgentIssueRun;
            await tx
              .update(issues)
              .set({
                executionRunId: sameAgentIssueRun.id,
        ...
        const sameAgentDeferred = await tx
          .select()
          .from(agentWakeupRequests)
          .where(
            and(
          ...
          .orderBy(asc(agentWakeupRequests.requestedAt))
          .then((rows) => rows.find((row) => wakeupRequestIssueId(row) === issue.id) ?? null);

        if (sameAgentDeferred) {
          const sameAgentDeferredPayload = parseObject(sameAgentDeferred.payload);
          const sameAgentDeferredContext = parseObject(sameAgentDeferredPayload[DEFERRED_WAKE_CONTEXT_KEY]);
          const mergedDeferredContext = mergeCoalescedContextSnapshot(
            sameAgentDeferredContext,
            enrichedContextSnapshot,
          );
          const mergedDeferredPayload = {
            ...sameAgentDeferredPayload,
            ...(payload ?? {}),
            issueId,
            [DEFERRED_WAKE_CONTEXT_KEY]: mergedDeferredContext,
          };
          ...
          await tx
            ...
            .set({
              payload: mergedDeferredPayload,
              coalescedCount: (sameAgentDeferred.coalescedCount ?? 0) + 1,
              updatedAt: new Date(),
            })
            .where(eq(agentWakeupRequests.id, sameAgentDeferred.id));
