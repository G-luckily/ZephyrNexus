...
import { activityLog, agents, companies, costEvents, heartbeatRuns, issues, projects } from "@zephyr-nexus/db";
...
export function costService(db: Db) {
  return {
    ...
    /** Sum cost (cents) for a single issue. Used by per-issue budget guard. */
    getSpentCentsForIssue: async (companyId: string, issueId: string): Promise<number> => {
      const rows = await db
        .select({
    ...
    byModel: async (companyId: string, range?: CostDateRange) => {
      ...
      const modelExpr = sql<string>`coalesce((${heartbeatRuns.usageJson} ->> 'model'), 'unknown')`;
      const providerExpr = sql<string>`coalesce((${heartbeatRuns.usageJson} ->> 'provider'), 'unknown')`;

      return db
        .select({
          model: modelExpr,
          provider: providerExpr,
          costCents: sql<number>`coalesce(sum(round(coalesce((${heartbeatRuns.usageJson} ->> 'costUsd')::numeric, 0) * 100)), 0)::int`,
        ...
        .from(heartbeatRuns)
        .where(and(...conditions))
        .groupBy(modelExpr, providerExpr)
        .orderBy(desc(sql`coalesce(sum(round(coalesce((${heartbeatRuns.usageJson} ->> 'costUsd')::numeric, 0) * 100)), 0)::int`));
    },
