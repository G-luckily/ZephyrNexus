...
import type { AdapterRuntimeServiceReport } from "@zephyr-nexus/adapter-utils";
import type { Db } from "@zephyr-nexus/db";
import { workspaceRuntimeServices } from "@zephyr-nexus/db";
...
function buildWorkspaceCommandEnv(input: {
  ...
  env.ZEPHYR_WORKSPACE_CWD = input.worktreePath;
  env.ZEPHYR_WORKSPACE_PATH = input.worktreePath;
  env.ZEPHYR_WORKSPACE_WORKTREE_PATH = input.worktreePath;
  env.ZEPHYR_WORKSPACE_BRANCH = input.branchName;
  env.ZEPHYR_WORKSPACE_BASE_CWD = input.base.baseCwd;
  env.ZEPHYR_WORKSPACE_REPO_ROOT = input.repoRoot;
  env.ZEPHYR_WORKSPACE_SOURCE = input.base.source;
  env.ZEPHYR_WORKSPACE_REPO_REF = input.base.repoRef ?? "";
  env.ZEPHYR_WORKSPACE_REPO_URL = input.base.repoUrl ?? "";
  env.ZEPHYR_WORKSPACE_CREATED = input.created ? "true" : "false";
  env.ZEPHYR_PROJECT_ID = input.base.projectId ?? "";
  env.ZEPHYR_PROJECT_WORKSPACE_ID = input.base.workspaceId ?? "";
  env.ZEPHYR_AGENT_ID = input.agent.id;
  env.ZEPHYR_AGENT_NAME = input.agent.name;
  env.ZEPHYR_COMPANY_ID = input.agent.companyId;
  env.ZEPHYR_ISSUE_ID = input.issue?.id ?? "";
  env.ZEPHYR_ISSUE_IDENTIFIER = input.issue?.identifier ?? "";
  env.ZEPHYR_ISSUE_TITLE = input.issue?.title ?? "";
  return env;
}
