...
export async function initializeProjectWorkspaceStarterFiles(input: {
  cwd: string | null;
  projectName: string;
  ...
  const descriptionLine = readNonEmptyString(input.projectDescription) ?? "Project workspace initialized by Paperclip.";
...
1. Add project requirements to \`docs/PROJECT.md\`
2. Put implementation files under \`src/\`
3. Assign an agent task to start work in this directory
`;
  const projectDoc = `# ${input.projectName}
...
- This directory is the primary local workspace for the project.
- Keep planning notes in this file or adjacent docs.
- Store implementation code under \`src/\`.
`;
  const tasksDoc = `# Initial Tasks

- Clarify the project scope
- Add architecture notes
- Create the first implementation task for an agent
`;

  await Promise.all([
    writeFileIfMissing(path.join(cwd, "README.md"), readme),
    writeFileIfMissing(path.join(cwd, "docs", "PROJECT.md"), projectDoc),
    writeFileIfMissing(path.join(cwd, "docs", "TASKS.md"), tasksDoc),
  ]);
}
