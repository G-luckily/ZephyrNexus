...
describe("ui branding", () => {
  it("detects worktree mode from ZEPHYR_IN_WORKTREE", () => {
    expect(isWorktreeUiBrandingEnabled({ ZEPHYR_IN_WORKTREE: "true" })).toBe(true);
    expect(isWorktreeUiBrandingEnabled({ ZEPHYR_IN_WORKTREE: "1" })).toBe(true);
    expect(isWorktreeUiBrandingEnabled({ ZEPHYR_IN_WORKTREE: "false" })).toBe(false);
  });
  ...
  it("rewrites the favicon block for worktree instances only", () => {
    const branded = applyUiBranding(TEMPLATE, { ZEPHYR_IN_WORKTREE: "true" });
