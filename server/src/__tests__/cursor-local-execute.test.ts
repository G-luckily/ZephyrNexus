...
import { execute } from "@zephyr-nexus/adapter-cursor-local/server";

async function writeFakeCursorCommand(commandPath: string): Promise<void> {
  const script = `#!/usr/bin/env node
const fs = require("node:fs");

const capturePath = process.env.ZEPHYR_TEST_CAPTURE_PATH;
const payload = {
    ...
    try {
      const result = await execute({
        ...
        config: {
          ...
          model: "auto",
          env: {
            ZEPHYR_TEST_CAPTURE_PATH: capturePath,
          },
      ...
      expect(capture.prompt).toContain("ZEPHYR_API_KEY");
      expect(invocationPrompt).toContain("Paperclip runtime note:");
      expect(invocationPrompt).toContain("ZEPHYR_API_URL");
    } finally {
