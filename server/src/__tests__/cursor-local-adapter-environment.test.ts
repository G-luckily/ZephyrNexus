...
import { testEnvironment } from "@zephyr-nexus/adapter-cursor-local/server";
...
const outPath = process.env.ZEPHYR_TEST_ARGS_PATH;
if (outPath) {
    ...
    const result = await testEnvironment({
      ...
      config: {
        ...
        env: {
          ...
          ZEPHYR_TEST_ARGS_PATH: argsCapturePath,
