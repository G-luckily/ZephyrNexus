...
import { agents as agentsTable, companies, costEvents, heartbeatRuns } from "@zephyr-nexus/db";
...
import { ensureOpenCodeModelConfiguredAndAvailable } from "@zephyr-nexus/adapter-opencode-local/server";
...
export function agentRoutes(db: Db) {
  ...
  const strictSecretsMode = process.env.ZEPHYR_SECRETS_STRICT_MODE === "true";
  ...
  function toLeanOrgNode(node: Record<string, unknown>): Record<string, unknown> {
    const reports = Array.isArray(node.reports)
      ...
      : [];
    const adapterType = String(node.adapterType ?? "");
    const costTier = HIGH_COST_ADAPTER_TYPES.has(adapterType) ? "high" : "normal";
    return {
      id: String(node.id),
  ...
  router.get("/companies/:companyId/agents/:id/budget-summary", async (req, res) => {
    const companyId = req.params.companyId as string;
    const id = req.params.id as string;
    const agent = await svc.getById(id);
    if (!agent) {
    ...
    // Fetch recent cost events for this agent
    ...
    // Fetch blocked runs (monthly budget exceeded)
    const blockedRuns = await db
      ...
      .where(and(
        ...
        eq(heartbeatRuns.errorCode, "budget_exceeded")
      ))
    ...
    res.json({
      ...
      status: agent.spentMonthlyCents >= agent.budgetMonthlyCents && agent.budgetMonthlyCents > 0 ? "exceeded" : "ok",
      recentEvents,
      blockedRuns,
    });
  });
  ...
  router.get("/companies/:companyId/live-runs", async (req, res) => {
    ...
    if (minCount > 0 && liveRuns.length < minCount) {
      ...
      res.json([...liveRuns, ...recentRuns].map((run) => {
        const telemetry = buildExecutionTelemetry({
          ...
          fallbackAgentName: run.agentName,
        });
        return {
          ...run,
          workflow: telemetry.workflow,
          executionState: telemetry.state,
          retry: telemetry.retry,
          humanCheckpoint: telemetry.humanCheckpoint,
          browserLogSummary: telemetry.browserLogSummary,
          verification: telemetry.verification,
          outputArtifacts: telemetry.outputArtifacts,
          usedSkills: telemetry.skills,
        };
      }));
      return;
    }
